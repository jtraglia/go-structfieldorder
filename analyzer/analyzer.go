...
import (
	...
	"github.com/jtraglia/go-structfieldorder/internal/fields"
	"github.com/jtraglia/go-structfieldorder/internal/pattern"
)

type analyzer struct {
	include pattern.List `structfieldorder:"optional"`
	exclude pattern.List `structfieldorder:"optional"`

	fieldsCache   map[types.Type]fields.StructFields
	fieldsCacheMu sync.RWMutex `structfieldorder:"optional"`

	typeProcessingNeed   map[string]bool
	typeProcessingNeedMu sync.RWMutex `structfieldorder:"optional"`
}
...
func NewAnalyzer(include, exclude []string) (*analysis.Analyzer, error) {
	...
	return &analysis.Analyzer{ //nolint:structfieldorder
		Name:     "structfieldorder",
...
4ex: 
	github.com/jtraglia/go-structfieldorder/v3/analyzer\.<anonymous>
	github.com/jtraglia/go-structfieldorder/v3/analyzer\.TypeInfo`)
...
func (a *analyzer) processStruct(
	...
	ordered, correctOrder := a.litOrderedFields(lit, structTyp, !isSamePackage)
	if !ordered {
		pos := lit.Pos()
		return &pos, fmt.Sprintf("Field order should be: %s", correctOrder.String())
	}
...
func (a *analyzer) litOrderedFields(
	lit *ast.CompositeLit,
	typ *types.Struct,
	onlyExported bool,
) (bool, fields.StructFields) {
	...
	return f.OrderedFields(lit, onlyExported)
}
